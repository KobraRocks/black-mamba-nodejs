...
- Events (resources = 'events')
  ...
  - slots (GET /events/:id/slots): returns available start times for a given date and viewer tz_offset
  - create/update/destroy: organizer‑only (requires session user)
  - mine (GET /events?mine=1): list event types owned by current user
  - public by slug:
    - GET /events/s/:slug — show public event type
    - GET /events/s/:slug/slots — slots by slug
- EventBookings (resources = 'event_bookings')
...
Views
- views/events/index.js — list event types
- views/events/show.js — event type detail
